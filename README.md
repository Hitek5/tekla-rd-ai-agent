...
## Что нового в 0.2 (готовность к закрытому контуру)

Версия 0.2 превращает скелет в пилото-готовый агент. Полный список — в [CHANGELOG.md](CHANGELOG.md).

- **Замкнутый агент-цикл.** Новый `POST /agent/plan`: модель *предлагает* типизированный tool call (Pydantic-схемы, зеркало C#-DTO), оркестратор валидирует аргументы и прогоняет через policy в режиме dry-run — ничего не выполняя.
- **Криптографические approval-токены.** HMAC-подпись, привязка к (tool, args, user, project), одноразовость, срок жизни. Выпуск — через `POST /approvals` (отдельный ключ согласующего). C# host **повторно** проверяет подпись тем же секретом.
- **Tamper-evident аудит.** Журнал — хеш-цепочка (`seq` + `prev_hash`); целостность проверяется `GET /audit/verify`.
- **Жёсткий скрининг промптов.** NFKC-нормализация, свёртка гомоглифов, удаление невидимых символов, паттерны RU+EN.
- **Аутентификация и лимиты API.** Bearer `API_KEY`, лимит размера запроса и rate-limit.
- **RAG на BM25** вместо наивного пересечения токенов: IDF, насыщение TF, нормировка длины, фильтр по метаданным, цитаты. Чистый Python — ноль доп. зависимостей для КСПД.
- **VRAM-пресеты 12/16/24 ГБ** с запускаемыми командами + `scripts/serve_model.py`: автоподбор по `nvidia-smi` и **проверка SHA-256 модели** по `manifest.json` перед стартом.
- **Чек-лист готовности:** [docs/airgap-readiness-checklist.md](docs/airgap-readiness-checklist.md).
